...
class TastyDoobieScanner(
  ...
  private def walk(tree: Tree, out: ListBuffer[DiscoveredIntegration], method: MethodRef): Unit = tree match {
    ...
    case Select(Apply(TypeApply(Select(frag, q), _), _), terminal) if nm(q, "query") && isReadTerminal(terminal) =>
      sqlFrom(frag).foreach(sql => out += mkIntegration(method, DataAccessType.Read, sql))
    ...
    case Select(Select(frag, u), r) if nm(u, "update") && nm(r, "run") =>
      sqlFrom(frag).foreach(sql => out += mkIntegration(method, DataAccessType.Write, sql))
  ...
  /** Extract SQL from the fragment tree by collecting all string literal parts.
    *
    * For sql"..." interpolation, the TASTy tree contains a StringContext.apply(...)
    * call with the SQL template parts as string literals inside a SeqLiteral.
    * We collect all string literals from the fragment tree and join them.
    */
  private def sqlFrom(tree: Tree): Option[String] = {
