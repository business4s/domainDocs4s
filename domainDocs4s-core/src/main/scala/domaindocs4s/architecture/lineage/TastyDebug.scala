...
object TastyDebug {
  ...
  def dumpMethod(packageName: String, className: String, methodName: String)(using ctx: Context): String = {
    ...
    val cls = allClasses.find(c => c.name.toString.stripSuffix("$") == className)
      .getOrElse(throw new IllegalArgumentException(
        s"Class $className not found in $packageName. Available: ${allClasses.map(_.name).mkString(", ")}"))

    val method = cls.declarations.collectFirst {
      ...
      s"Method $methodName not found in $className. Available: ${cls.declarations.map(_.name).mkString(", ")}"))
